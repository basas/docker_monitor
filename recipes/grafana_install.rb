...
directory '/etc/opt/docker/grafana/provisioning/dashboards' do
  mode        '0755'
  recursive   true
  action      :create
end

directory '/etc/opt/docker/grafana/provisioning/datasources' do
  mode        '0755'
  recursive   true
  action      :create
end
...
template '/etc/opt/docker/grafana/provisioning/datasources/datasource_prometheus.yaml' do
  ...
  mode      '0755'
  notifies  :run, 'execute[restart_grafana]', :delayed
end
...
docker_container 'grafana' do
  ...
  volumes         ['/etc/opt/docker/grafana/provisioning:/etc/grafana/provisioning',\
                   '/etc/opt/docker/grafana/provisioning/dashboards:/var/lib/grafana/dashboards']
  action          :run
end

execute 'restart_grafana' do
